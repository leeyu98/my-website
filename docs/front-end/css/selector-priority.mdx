...
<Author cost={8} publish="2023-10-7 15:54" update="2024-6-4 14:44" />
...
<details>
  <summary>如果 `ID` 选择器全局不唯一会如何？</summary>
  <p>
    首先功能上不会受到影响，类似 `class` 选择器，所有匹配的 `id` 都能被选择，样式也会生效； 使用
    `document.querySelectorAll` 也能获取到一个包含所有匹配元素的列表。
  </p>
</details>
...
| 选择器                                                              | 权重 |
| ------------------------------------------------------------------- | ---- |
| 行内样式 `inline style`                                             | 1000 |
| ID 选择器                                                           | 100  |
| 类选择器 `class`、属性选择器 `attibutes`、伪类选择器 `pseudo-class` | 10   |
| 元素选择器 `element`、伪元素选择器 `pseudo-element`                 | 1    |
| 关键字 `!important`                                                 | 最高 |
