...
函数的`__proto__` 指向该函数构造函数的 `prototype`，那 `prototype` 到底是什么呢？
...
如果按照一般规则把 `Function.prototype` 看成函数的话，那么应该有 `Function.prototype.__proto__ === Function.prototype`那又会出现像 `Object` 中一样的循环引用问题，所以 JavaScript 规定`Function.prototype.__proto__ === Object.prototype`，这样就使原型链有了唯一的终点 `null`。

## 附-原型链图

![原型链](./images/proto-chain.jpg)
