...
抽象相等 (`Abstract Equality`)，也称宽松相等 (`Loose Equality`)，操作符为 `==` 和 `!=`。特点是<Emphasis>会对类型不同的两个操作数进行强制类型转换</Emphasis>。
...
:::warning 违反规则的特例
`document.all` 被视为 `undefined`，所以 `document.all == undefined` 和 `document.all == null` 为 `true`，但是 `document.all === null` 和 `document.all === undefined` 都为 `false`。
:::
...
1. 调用该对象的 `@@toPrimitive(hint)` 方法 (以 `default` 为 `hint`)；
2. 调用该对象的 `valueOf()` 方法；
