...
:::caution info
加载源数据的方式<Emphasis>只能且必须</Emphasis>选择 `lovCode` 与 `transport` 一种：`lovCode` 的配置更为简洁， `transport` 配置更为灵活。<br/>如果使用 `JS/JSX` 可能不会有错误提示，同时配置的话 `read` 优先级更高。
:::
...
| lovUrl     | `string`        | <Highlight>可选</Highlight> 加载源数据的接口地址。支持参数有：<br/> - `lov` : 普通值集，内置查询接口<br/> - `sql` : `sql` 值集，内置查询接口<br/> - `string`: `url` 值集或其他接口 | `lov` 普通值集 |
