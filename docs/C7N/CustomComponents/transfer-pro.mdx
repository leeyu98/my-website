...
```tsx
Modal.open({
  title: intl.get(`${intlPrefix}.title.assignment`).d('分配'),
...
| modal       | `any`                                                                   | <Highlight>可选</Highlight> 使用 `Modal.open` 打开弹窗，默认会注入该参数，组件内部将目标数据结果注入到 `onOk` 回调中 |                                |
...
```ts
/** @description lovCode 与 read 二选一不能共存 */
export type ConfigProps = {
  primaryKey: string;
  textField: string;
  valueField?: string;
  lovUrl?: string;
} & ({ lovCode: string; read?: undefined } | { lovCode?: undefined; read: TransportType });
```

:::info 注意
加载源数据的方式<Emphasis>只能且必须</Emphasis>选择 `lovCode` 与 `read` 一种：`lovCode` 的配置更为简洁， `read` 配置更为灵活。<br/>如果使用 `JS/JSX` 可能不会有类型错误提示，同时配置的话 `read` 优先级更高。
:::

| 参数       | 类型            | 说明                                                                                                                                                                                                                 | 默认值         |
| :--------- | :-------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------- |
| primaryKey | `string`        | <Highlight color="red">必输</Highlight> 主键，用于数据标识                                                                                                                                                           | -              |
| textField  | `string`        | <Highlight color="red">必输</Highlight> 作为主要展示字段                                                                                                                                                             | -              |
| lovCode    | `string`        | <Highlight color="red">必输</Highlight> 值集编码，用于加载源数据                                                                                                                                                     | -              |
| read       | `TransportType` | <Highlight color="red">必输</Highlight> 详细配置参见 [Transport](https://open.hand-china.com/choerodon-ui/zh/procmp/dataset/dataset#transport)，优先级高于 `lovCode`                                                 | -              |
| valueField | `string`        | <Highlight>可选</Highlight> 作为次要展示字段                                                                                                                                                                         | `value`        |
| lovUrl     | `string`        | <Highlight>可选</Highlight> 加载源数据的接口地址。支持参数有：<br/> - `lov` : 普通值集，内置接口 `/hpfm/v1/lovs/data`<br/> - `sql` : `sql` 值集，内置接口 `/hpfm/v1/lovs/sql/data`<br/> - `string`: 自定义接口 `url` | `lov` 普通值集 |
...
| 参数       | 类型             | 说明                                                                   | 默认值                               |
| :--------- | :--------------- | :--------------------------------------------------------------------- | :----------------------------------- |
| data       | `string`         | <Highlight color="red">必输</Highlight> 目标数据                       | -                                    |
| transport  | `TransportProps` | <Highlight color="red">必输</Highlight> 目标数据源配置，仅 `read` 有效 | -                                    |
| primaryKey | `string`         | <Highlight>可选</Highlight> 仅用于数据匹配                             | 默认与 `ConfigProps.primaryKey` 一致 |

:::caution 注意
`TransferDSProps` 与 `TargetDSProps` 对返回数据的要求：<br/>
`Array` 或者 `{ content: Array }`，其他返回结果需要通过配置 `transport.read.transformResponse` 自行格式转换。
:::
