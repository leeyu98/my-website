...
| afterQuery       | `(params: any) => any`             | <Highlight>可选</Highlight> 哪些字段定义在 queryFields 或者 queryDataSet 中但不需要渲染的字段   | `[]`       |
| excludeFields    | `string[]`                         | <Highlight>可选</Highlight> 点击查询按钮之后的回调，参数中可以拿到所有的查询参数                | `() => {}` |
    ...
    <div className={wrapperClassName} style={{ marginTop: withSpace ? 0 : -10, ...wrapperStyle }}>
      ...
      <Table
        ...
        pagination={{
          ...
          showTotal: (total: number, range: [number, number], _counting: any, page: number) => {
            return (
              <span>
                共 {total} 条记录 第 {page}/{range?.[0]} 页
              </span>
            );
          },
        }}
